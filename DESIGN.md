...
The Workflows route family begins with four route-backed tabs in a stable order: **All Workflows**, **All Schedules**, **All Incoming Webhooks**, and **All Runs**. All Runs renders the workspace execution ledger without replacing the other first-class workflow-management routes. The page header offers one **Add workflow** menu for template and custom creation, plus a route-backed, searchable **Workflow guide**. Inside a selected workflow, **Overview**, **Agents**, **Capabilities**, and **Runs** are the four visible inspection tabs. **Edit** opens the route-backed Settings view, which retains `tab=settings` deep-link compatibility and provides a Back to workflow action; the other existing `tab=agents`, `tab=capabilities`, and `tab=runs` links select their inline detail panels. The workflow header states **Ready**, **Blocked**, or **Inactive** before presenting text-labelled **Edit**, **Schedules**, **Webhooks**, and **Launch**, **Review readiness**, or **Activate** actions. Successful definition, Agent, tag, and activation changes expose **Undo**. Schedules and Incoming webhooks open their workflow-filtered drawer tables; creation begins only from each drawer's Create action. On desktop the workflow search is integrated into the compact library pane; compact layouts expose search only in the library view. Pressing `/` focuses workflow search, library rows support Arrow Up, Arrow Down, Home, and End, and Control/Command+Enter opens launch review without confirming a run.
