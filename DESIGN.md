...
Workflows and MCP Catalog use the shared catalog split. One bordered surface contains a divided library and detail pane with a `32rem` minimum height. The shared default activates the split at `lg`, with a `minmax(18rem, 22rem)` library and a detail pane that fills the remaining width. Task-heavy routes may use a wider custom threshold when the desktop sidebar and detail controls would otherwise crowd the content. Workflows activates its split at `1440px`, with a `minmax(20rem, 24rem)` library. This preserves the library on a 1920px display at 125% browser zoom while switching to the drill-in view around 150% zoom. A task-heavy route may also opt into the bounded desktop variant: the route canvas stays fixed, the library and active detail body scroll independently, and the detail header remains visible. Below the route's selected split threshold, document scrolling resumes to avoid nested touch-scroll regions; only the route-selected pane is visible, and detail provides a Back action that returns to the library and restores focus to its selected row. Desktop may preview the first visible item without writing selection state to the URL. Shared primitives also own list headers, row padding and selection, loading and empty states, detail headers, detail-body padding and tone, and the discovery-to-surface gap. Page-specific filters, metadata, actions, and detail fields remain feature-owned.

The Workflows route family begins with three route-backed workspace tabs in a stable order: **Workflows**, **Schedules**, and **Runs**. Runs renders the workspace execution ledger without replacing the other first-class workflow-management routes. Incoming webhooks belong to a workflow and are managed only from that selected workflow's **Webhooks** detail tab; the former aggregate route remains available for bookmarked-link compatibility but is not part of workspace tab navigation. The page header offers one direct **Add workflow** action plus route-backed, searchable **Workflow help**. Inside a selected workflow, **Overview**, **Capabilities**, **Schedules**, **Webhooks**, and **Runs** are visible route-backed tabs. Overview owns the workflow's Agent assignment summary and editor so the same fact is not repeated in a separate tab. **Edit** opens the route-backed Settings view and provides a Back to workflow action. The workflow header states **Ready**, **Blocked**, or **Inactive** before presenting text-labelled **Edit** and **Launch**, **Review readiness**, or **Activate** actions. Successful definition, Agent, tag, and activation changes expose **Undo**. Schedule and webhook creation begins from each workflow detail tab's Create action. On desktop the workflow search is integrated into the workflow library pane; compact layouts expose search only in the library view. Pressing `/` focuses workflow search, library rows support Arrow Up, Arrow Down, Home, and End, and Control/Command+Enter opens launch review without confirming a run.

Workflow prompts are saved and launched as plain text. Agent chat and workflow prompt fields support `@target` autocomplete, which inserts a readable `@target[Target name]` mention without creating an Agent or Workflow target binding. Other prompt-reference types and runtime template parameters are not supported. Schedule and incoming-webhook creation use the shared medium dialog width (`36rem`) with a bounded viewport height; the underlying task drawer remains visible as context.
...
- **Canonical component:** Textual lifecycle and health states use `StatusBadge`.
  Unlabeled colored dots are reserved for presence indicators and labeled metric
  legends; they are not paired with a separate status word.
...
- **Sizing mode:** `DataTable` is content-sized by default. Stable comparison tables opt into `table-fixed` and declare all responsive widths in one `colgroup`; cells do not carry a competing width model. Responsive grid ledgers use one named grid-template constant for the header and every row.
- **Track priority:** Identity and description receive the flexible remainder. Metadata tracks use reviewed bounds, while status, toggle, count, and action tracks remain compact and stable. Hiding a column updates the track model at the same breakpoint and preserves necessary facts as labeled compact metadata.
...
- **Top-level discovery:** Collection pages use `DiscoveryFilterBar` with a labeled `PageSearchInput`, zero or more typed filter-group definitions created by `createDiscoveryFilterGroup`, and a polite result summary. Standalone collection summaries use `CollectionResultSummary`, the same quiet `type-caption` live-region treatment; they are not uppercase, outlined, or presented as pills. `DiscoveryFilterBar` and nested resource search both compose `SearchFilterFrame`, the canonical bordered paper surface with `16px` padding, a restrained shadow, `12px` gaps, and stable `44px` controls. Search is the dominant flexible field while categorical controls stay approximately `11rem` to `14rem` wide.
