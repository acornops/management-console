...
Workflow detail has one permanent **Overview** canvas for the definition, Agents, access, approvals, and prompt. The compact header presents five icon actions in a stable order: Run activity, Schedules, Incoming webhooks, Edit, and Launch or Activate. The first three are operational views; a subtle separator distinguishes definition and launch actions. Every icon has a concise accessible name and a hover or focus tooltip; blocker details belong in the tooltip or opened task surface, not the button name. Run activity, Agents, access review, and Edit open focused right-side drawers while Overview stays mounted behind them. Schedules and Incoming webhooks open workflow-filtered drawer tables; their Create actions open modal forms above the drawer. Existing `tab=agents`, `tab=capabilities`, `tab=settings`, and `tab=runs` links remain valid as drawer-opening compatibility states. Legacy schedule and incoming-webhook routes remain valid without reintroducing navigation tabs.
