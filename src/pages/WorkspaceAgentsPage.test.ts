...
describe('WorkspaceAgentsPage surface', () => {
  it('uses a lightweight operator catalog with drawer-based management', () => {
    ...
    ['Assignment checks', 'AssignmentCheck', '<AssignmentCheck label="Targets" value={targetSummary} />', '<AssignmentCheck label="Data available" value={getAgentContextSummary(agent)} />', 'getAgentTargetSummary', 'getAgentContextSummary', 'targetSummary'].forEach((snippet) => expect(agentsCatalog).not.toContain(snippet));
    ['Targets', 'Data available'].forEach((snippet) => expect(agentsPage).not.toContain(snippet));
    expect(agentsPage).toContain('Approvals');
    expect(agentsPage).toContain('Workflows');
    ['All Kubernetes clusters', 'Workspace metadata', 'Target inventory', 'Selected chat sessions'].forEach((snippet) => expect(agentsCatalog).not.toContain(snippet));
    ...
    ['Target access', 'Context access', 'Approval gates', 'approval gate', 'Write access:'].forEach((snippet) => expect(agentsPage).not.toContain(snippet));
    expect(agentsPage).toContain('aria-expanded={expanded}');
    expect(agentsPage).toContain('Run readiness');
    ['capability entries', 'Tools allowed', 'Profile</div>'].forEach((snippet) => expect(agentsPage).not.toContain(snippet));
    expect(agentsPage).toContain('Open profile');
    expect(agentsPage).toContain('Configuration');
    ['Manage agent', 'More actions', 'Access, triggers, and history', 'Primary action', 'Manage definition', 'Operations'].forEach((snippet) => expect(agentsPage).not.toContain(snippet));
