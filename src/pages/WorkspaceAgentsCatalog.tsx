...
export function getAgentCapabilitySummary(agent: AgentDefinition, t: TFunction): string {
  return [
    t('agentsWorkflows.agents.capabilityCounts.mcpServer', {
      count: agent.mcpServers.length
    }),
    t('agentsWorkflows.agents.capabilityCounts.tool', {
      count: agent.tools.length
    }),
    t('agentsWorkflows.agents.capabilityCounts.skill', {
...
export function getAgentWorkflowAssignmentSummary(agent: AgentDefinition, t: TFunction): AgentWorkflowAssignmentSummary {
  ...
  return {
    ...
    overflowLabel:
      count > 1
        ? t('agentsWorkflows.agents.workflowAssignment.overflow', {
            count: count - 1
          })
        : undefined
  };
...
const catalogGridClass =
  'pointer-events-none relative z-10 grid min-w-0 grid-cols-2 gap-x-5 gap-y-3 px-4 py-3.5 lg:grid-cols-[minmax(14rem,1.4fr)_7rem_minmax(12rem,0.8fr)_minmax(13rem,1fr)_2.75rem] lg:items-center';
...
export const WorkspaceAgentsCatalog: React.FC<WorkspaceAgentsCatalogProps> = ({
  ...
  const filterOptions = React.useMemo<Array<DiscoveryFilterOption<AgentFocusFilter>>>(
    () => [
      {
        value: 'all',
        label: t('agentsWorkflows.agents.filters.all'),
        count: agents.length
      },
      {
        value: 'active',
        label: t('agentsWorkflows.agents.status.active'),
        count: agents.filter((agent) => agent.status === 'active').length
      },
      {
        value: 'draft',
        label: t('agentsWorkflows.agents.status.draft'),
        count: agents.filter((agent) => agent.status === 'draft').length
      },
      {
        value: 'disabled',
        label: t('agentsWorkflows.agents.status.disabled'),
        ...
        <DiscoveryFilterBar
          ...
          resultSummary={
            loading
              ? t('common.loading')
              : hasActiveFilters
              ? t('agentsWorkflows.agents.resultCount', {
                  visible: visibleAgents.length,
                  total: agents.length
                })
              : t('agentsWorkflows.agents.totalCount', { count: agents.length })
          }
          filters={[
            createDiscoveryFilterGroup<AgentFocusFilter>({
              id: 'status',
      ...
      <CollectionState
          ...
          <div role="status" aria-live="polite">
            ...
            <ul aria-hidden="true" className="divide-y divide-ui-border">
              {Array.from({ length: 3 }).map((_, index) => (
                <li
                  key={index}
                  className="grid min-h-28 grid-cols-[minmax(0,1fr)_5rem] items-center gap-5 px-4 py-3.5 lg:grid-cols-[minmax(14rem,1.4fr)_7rem_minmax(12rem,0.8fr)_minmax(13rem,1fr)_2.75rem]"
                >
        ...
        empty={
          <EmptyState
            embedded
            icon={<ICONS.Bot />}
            title={t('agentsWorkflows.agents.emptyTitle')}
            description={t(canManageAgents ? 'agentsWorkflows.agents.emptyBody' : 'agentsWorkflows.agents.emptyReadOnlyBody')}
          />
        }
        ...
        <ul role="list" className="divide-y divide-ui-border">
          {visibleAgents.map((agent) => {
            ...
            return (
              <li
                key={agent.id}
                data-agent-catalog-row={agent.id}
                className={`group relative isolate min-w-0 overflow-hidden transition-colors duration-200 ${
                  ...
                  <CatalogCell label={t('agentsWorkflows.agents.fields.identity')} className="col-span-2 lg:col-span-1">
                    <span className="block min-w-0 pr-8 lg:pr-0">
                      <span className="flex min-w-0 flex-wrap items-center gap-2">
                        <span className="type-row-title break-words text-ui-text transition-colors group-hover:text-accent-strong group-focus-within:text-accent-strong [overflow-wrap:anywhere]">
                          {agent.name}
                        </span>
                      </span>
