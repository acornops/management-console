  ...
  return (
    ...
    >
      ...
      {creationStep === 'details' ? (
          ...
          <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-6 custom-scrollbar">
            <div className="rounded-xl border border-ui-border bg-ui-bg px-4 py-4 text-sm font-medium leading-6 text-ui-text-muted">
              <div className="flex items-start gap-3">
                ...
                <p>{t('virtualMachines.list.connectBody')}</p>
              </div>
            </div>
            ...
            <section className="space-y-3">
              <label htmlFor="add-vm-name-input" className="block px-1 text-[11px] font-extrabold uppercase tracking-[0.18em] text-ui-text-muted">
                1. {t('virtualMachines.list.vmName')}
              </label>
              <input
            ...
            {errorMessage && (
              <div className="rounded-xl border border-status-danger/25 bg-status-danger-soft p-4 text-sm font-semibold text-status-danger-text">
                {errorMessage}
              </div>
            )}
          </div>
        ...
        <>
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-6 custom-scrollbar">
            <div className="rounded-xl border border-ui-border bg-ui-bg px-4 py-4 text-sm font-medium leading-6 text-ui-text-muted">
              <div className="flex items-start gap-3">
                <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg text-accent-strong">
                  <ICONS.Terminal className="h-4 w-4" />
            ...
            {installInstructions ? (
              <div className="rounded-xl border border-ui-border bg-ui-bg shadow-sm">
                <div className="flex items-center justify-between gap-3 px-4 pt-4">
                  <span className="text-[11px] font-extrabold uppercase tracking-widest text-ui-text-muted">2. {t('virtualMachines.list.installInstructions')}</span>
                  <button
                    type="button"
