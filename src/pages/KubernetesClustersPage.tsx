...
const CLUSTER_STATUS_FILTERS: ReadonlyArray<ClusterCatalogStatusFilter> = ['all', 'attention', 'healthy', 'not_installed'];
...
function clusterMatchesCatalogState(cluster: KubernetesCluster, query: string, status: ClusterCatalogStatusFilter, issueSummary?: ControlPlaneTargetIssueSummary): boolean {
  ...
  return [cluster.name, cluster.cluster, cluster.namespace, cluster.workspaceId].some((value) => value?.toLowerCase().includes(normalizedQuery));
}
...
export const KubernetesClustersPage: React.FC<KubernetesClustersPageProps> = ({
  ...
  const hasCompleteIssueSummaries = kubernetesClusters.every(
    (cluster) => Object.prototype.hasOwnProperty.call(issueSummaryByClusterId, cluster.id) && issueSummaryByClusterId[cluster.id] !== undefined
  );
  const catalogCounts = useMemo<Partial<Record<ClusterCatalogStatusFilter, number>>>(() => {
    ...
    counts.not_installed = kubernetesClusters.filter((cluster) => getAgentConnectionState(cluster) === 'not_installed').length;
    if (!hasCompleteCatalogCounts || !hasCompleteIssueSummaries) return counts;

    counts.attention = kubernetesClusters.filter((cluster) => clusterNeedsAttention(cluster, issueSummaryByClusterId[cluster.id])).length;
    counts.healthy = kubernetesClusters.filter(
      (cluster) => getAgentConnectionState(cluster) === 'connected' && !clusterNeedsAttention(cluster, issueSummaryByClusterId[cluster.id])
    ).length;
    return counts;
  ...
  const setCatalogState = useCallback(
    (nextState: ClusterCatalogRouteState) => {
  ...
  const updateCatalogState = useCallback(
    (patch: Partial<ClusterCatalogRouteState>) => {
  ...
  const handleStatusChange = useCallback(
    (nextStatus: ClusterCatalogStatusFilter) => {
      updateCatalogState({
  ...
  const handleDeleteKubernetesCluster = useCallback(
    async (cluster: KubernetesCluster) => {
  ...
  const loadClusterPage = useCallback(
    ({ cursor, filters, limit, signal }: { cursor?: string; filters: typeof catalogRequestFilters; limit: number; signal: AbortSignal }) =>
      workspaceId
        ? controlPlaneApi.listClustersForWorkspace(workspaceId, {
            limit,
            cursor,
            ...filters,
            signal
          })
        : Promise.resolve({ items: [] as KubernetesCluster[] }),
    [workspaceId]
  );
  ...
  useEffect(() => {
    ...
    setMetricLoadStateByClusterId((current) => Object.fromEntries(connectedClusters.map((cluster) => [cluster.id, current[cluster.id] === 'ready' ? 'ready' : 'loading'])));
    ...
    void Promise.all(
      ...
      .then((historyResults) => {
        ...
        setMetricLoadStateByClusterId(Object.fromEntries(connectedClusters.map((cluster) => [cluster.id, 'ready'])));
      })
      .catch((error) => {
        if (requestId === metricHistoryRequestSeqRef.current) {
          console.error('Failed loading cluster metric history', error);
          setMetricLoadStateByClusterId(Object.fromEntries(connectedClusters.map((cluster) => [cluster.id, 'error'])));
        }
      });
  ...
  const visibleClusters = useMemo(() => {
    const mergedClusters = loadedClusterPageItems.length > 0 ? mergeClustersById(loadedClusterPageItems, clientFilteredClusters) : clientFilteredClusters;
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col" data-cluster-catalog-scope={workspaceId ? 'workspace' : 'global'}>
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto custom-scrollbar">
        <Dashboard
          ...
          controls={
            hasClusterInventory || hasActiveFilter ? (
              <DiscoveryFilterBar
                ...
                resultSummary={
                  catalogPending
                    ...
                    : t('dashboard.clusterCount', {
                        count: totalClusterCount ?? kubernetesClusters.length
                      })
                }
                filters={[
                  createDiscoveryFilterGroup<ClusterCatalogStatusFilter>({
                    id: 'status',
          ...
          catalogFooter={
            workspaceId ? (
              <div ref={clusterCollection.sentinelRef} className="flex justify-center py-2">
                {nextCursor && (
                  <Button type="button" variant="secondary" onClick={() => void clusterCollection.loadMore()} disabled={isLoadingMore}>
