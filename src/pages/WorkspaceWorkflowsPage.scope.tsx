  ...
  return (
    <fieldset className="min-w-0">
      ...
      <details className="group mt-2 rounded-md border border-ui-border bg-ui-surface">
        <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3 px-3 py-2 type-body type-emphasis text-ui-text transition-colors hover:bg-ui-bg focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/25 [&::-webkit-details-marker]:hidden">
        ...
        <div className="border-t border-ui-border bg-ui-bg p-3">
          <TextInput value={query} onChange={(event) => setQuery(event.target.value)} placeholder={searchPlaceholder} aria-label={`Search ${label.toLowerCase()}`} aria-disabled={options.length === 0} disabled={options.length === 0} className="h-9 type-caption" />
            ...
            {filteredOptions.length > 0 ? filteredOptions.map((option) => (
              <label key={option.value} className="flex min-h-11 items-start gap-3 border-b border-ui-border/70 px-3 py-2 last:border-b-0">
                ...
                <span className="min-w-0">
                  <span className="block break-words type-body type-emphasis text-ui-text [overflow-wrap:anywhere]">{option.label}</span>
              ...
              <div className="px-3 py-4 type-body type-emphasis text-ui-text-muted">{emptyMessage}</div>
            )}
          </div>
          {selectedValues.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {selectedValues.map((selectedValue) => {
                ...
                return (
                  <Button key={selectedValue} type="button" variant="secondary" size="sm" onClick={() => onToggle(selectedOption || { value: selectedValue, label: selectedValue }, false)} className="inline-flex min-h-11 max-w-full items-center gap-1 rounded-full border border-ui-border bg-ui-surface px-2.5 py-1 text-ui-text-muted transition-colors hover:border-accent/40 hover:text-ui-text focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/25 sm:min-h-8">
