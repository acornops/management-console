          ...
          {selectedValues.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-2">
              {selectedValues.map((selectedValue) => {
                ...
                return (
                  <Button key={selectedValue} type="button" onClick={() => onToggle(selectedOption || { value: selectedValue, label: selectedValue }, false)} className="inline-flex min-h-11 max-w-full items-center gap-1 rounded-full border border-ui-border bg-ui-surface px-2.5 py-1 type-caption text-ui-text-muted transition-colors hover:border-accent/40 hover:text-ui-text focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/25 sm:min-h-8">
