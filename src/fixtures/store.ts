...
export function createFixtureState(): FixtureState {
  ...
  const virtualMachine = {
    ...
    pendingAgentAccessPolicy: null as null | { accessMode: 'read_only' | 'read_write'; restartServices: string[] },
    permissionMode: 'ask_before_changes' as const,
    permissionModeOverride: null,
    permissionModeSource: 'deployment_default' as const,
  ...
  return {
    ...
    targetMcpServers: [
      ...
      { id: 'fixture-vm-mcp', workspace_id: FIXTURE_IDS.workspace, target_id: FIXTURE_IDS.virtualMachine, target_type: 'virtual_machine', server_name: 'AcornOps VM Tools', server_url: 'builtin://agentv', enabled: true, auth_type: 'none', credential_mode: 'none', connection_status: 'ok', last_discovery_at: NOW, last_discovery_error: null, revision: 1, tools: [
        ...
        { name: 'restart_service', description: 'Restart an allowed Linux service under VM run policy', capability: 'write', version: '2.0.0', source: 'builtin', enabled: true, mcp_server_url: 'builtin://agentv', timeout_ms: 30000 }
      ] }
    ],
