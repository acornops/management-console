...
export const ResourceMetricInline: React.FC<{
  ...
  return (
    <div data-resource-metric-inline="true" className="min-w-0">
      <p className="type-micro-label mb-0.5">{t(`workloads.metricLabels.${label}`, { defaultValue: label })}</p>
      <p className="type-row-title break-words" title={value}>
        {value}
      </p>
    </div>
  );
...
export const InfrastructureRow: React.FC<{
  ...
  const { t } = useTranslation();
  const rowClassName = classNames(resourceRowGridClass, onClick && 'cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-accent/30');
  const content = (
    <>
      ...
      <div className={resourceMetricGridClass}>
        {metrics.length > 0 ? (
          metrics.slice(0, 4).map((metric) => <ResourceMetricInline key={metric.label} label={metric.label} value={metric.value} />)
        ) : (
...
export const DetailRow: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div className="grid grid-cols-[minmax(7rem,0.7fr)_minmax(0,1fr)] gap-4 border-b border-ui-border py-2 last:border-b-0">
    ...
    <span className="type-caption min-w-0 truncate text-right text-ui-text" title={value}>
      {value}
    </span>
  </div>
);
