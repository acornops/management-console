...
import { captureLogViewport, resolveRestoredLogScrollTop, type LogViewportSnapshot } from '@/features/kubernetes-cluster-detail/components/workloads/podLogViewport';
...
export const WorkloadDetailsDrawer: React.FC<WorkloadDetailsDrawerProps> = ({ selectedWorkload, canReadPodLogs, onClose, onAnalyzePod, onLoadPodLogs }) => {
  ...
  const selectedPodContainers = useMemo(() => (selectedWorkload?.type === 'Pod' ? selectedWorkload.containers || [] : []), [selectedWorkload]);
  const canShowLogs = Boolean(selectedWorkload?.type === 'Pod' && canReadPodLogs && onLoadPodLogs);
  const logContainerOptions: Array<SelectOption<string>> =
    selectedPodContainers.length === 0
  ...
  const loadPodLogs = useCallback(
    async (mode: 'manual' | 'follow' = 'manual') => {
      ...
      try {
        ...
        pendingLogViewportRef.current = logViewportRef.current ? captureLogViewport(logViewportRef.current, mode === 'follow') : null;
        setPodLogs(result);
      } catch (error) {
        if (logsRequestIdRef.current !== requestId) return;
  ...
  return (
    <SidePanel isOpen={Boolean(selectedWorkload)} onClose={closeDrawer} title={t('workloads.drawerTitle', { name: selectedWorkload?.name || '' })}>
            ...
            {(['details', 'logs'] as WorkloadDetailTab[]).map((tab) => (
              <Button
                ...
                className={`control-target ${classNames(
                  ...
                  activeDetailTab === tab ? 'bg-ui-surface text-accent-strong shadow-sm' : 'text-ui-text-muted hover:text-ui-text'
                )}`}
              >
          ...
          <>
              ...
              <div className="mt-3">
                ...
                {selectedWorkload?.type === 'Pod' && (
                  <>
                    ...
                    <DetailRow label={t('workloads.containers')} value={selectedPodContainers.length > 0 ? selectedPodContainers.join(', ') : '-'} />
                  </>
                )}
              </div>
            </section>
            ...
            {selectedWorkload?.type === 'Pod' && selectedWorkload.containerStatuses && selectedWorkload.containerStatuses.length > 0 && (
              <section className="border-t border-ui-border pt-5">
                <span className="type-label">{t('workloads.containerRuntime')}</span>
              ...
              <>
                <div className="border-y border-ui-border bg-ui-bg/60 px-4 py-3">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <label className="type-label">{t('workloads.container')}</label>
                      <Select<string>
                        value={logContainer}
                    ...
                    <div className="space-y-2">
                      <label className="type-label">{t('workloads.tail')}</label>
                      <Select<number>
                        value={logTailLines}
                  ...
                  <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                    <label className="inline-flex items-center gap-2 type-label">
                      <Checkbox checked={logPrevious} onChange={(event) => setLogPrevious(event.target.checked)} disabled={isPodLogsLoading} />
                ...
                <div className="overflow-hidden rounded-xl border border-ui-border bg-code-bg shadow-sm">
                  <div className="flex items-center justify-between gap-3 border-b border-code-text/10 px-4 py-3">
                    <div className="inline-flex min-w-0 items-center gap-2 type-label text-code-text">
                    ...
                    <div className="inline-flex items-center gap-2 type-caption type-emphasis text-code-text/70">
                      ...
                      {podLogs?.fetchedAt
                        ? t('workloads.fetchedAt', {
                            time: formatUserTime(podLogs.fetchedAt, {
                              includeTimeZone: true
                            })
                          })
                        : t('workloads.notLoaded')}
                    </div>
                  </div>
                  <pre
                    ref={logViewportRef}
