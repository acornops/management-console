...
import { KnowledgeBankSettingsPanel } from '@/features/kubernetes-cluster-detail/components/detail/views/KnowledgeBankSettingsPanel';
...
interface KnowledgeBankSettingsDialogProps {
  workspaceId: string;
  targetId: string;
...
export const KnowledgeBankSettingsDialog: React.FC<KnowledgeBankSettingsDialogProps> = ({
  workspaceId,
  targetId,
  tool,
  canEdit,
  ...
  return (
    <Dialog
      titleId="knowledge-bank-settings-dialog-title"
    ...
    >
      <div className="flex items-start justify-between gap-4 border-b border-ui-border bg-ui-bg px-6 py-4">
        <div className="min-w-0">
          <h3 id="knowledge-bank-settings-dialog-title" className="type-panel-title">{t('tools.knowledgeBank.settingsTitle')}</h3>
          <p className="type-caption mt-1 text-ui-text-muted">{t('tools.knowledgeBank.settingsBody')}</p>
        ...
        <button
          ...
          aria-label={t('tools.knowledgeBank.closeSettings')}
        >
          <X className="h-4 w-4" />
        </button>
      </div>
      {loading ? (
        <div className="flex min-h-[18rem] items-center justify-center">
          <InlineLoadingIndicator label={t('tools.knowledgeBank.loadingSettings')} />
        </div>
      ) : (
        <>
          {error && (
      ...
      <div className="flex items-center justify-end gap-3 border-t border-ui-border bg-ui-bg px-6 py-4">
        {!canEdit ? (
          ...
          <>
            <Button variant="secondary" size="sm" onClick={onClose} disabled={settingsSaving || savingTool}>{t('tools.knowledgeBank.cancel')}</Button>
