...
describe('target chat polish contracts', () => {
  ...
  it('keeps polish shared across transcript, composer, and trace surfaces', () => {
    ...
    expect(chatView).toContain('const title = activeSession && (activeSession.backendSessionId || activeSession.messages.length > 0) ? activeSession.name : t(\'chat.triageConsole\');');
    ...
    expect(chatSessionSync).toContain('if (session.hydrated && !hasTransientAssistantPlaceholder && !hasAssistantWithInProgressTrace)');
    ...
    expect(chatView).not.toContain("t('chat.newChatWaitingForRun')");
    expect(enLocale).not.toContain('newChatWaitingForRun');
    expect(zhLocale).not.toContain('newChatWaitingForRun');
    ...
    expect(chatView).toContain('await onEditLastUserMessage(messageId, nextContent);');
    ...
    expect(enLocale).not.toContain("reasoningSummary: 'Working through'");
    ...
    expect(zhLocale).not.toContain("preparingResponse: '正在准备回复...'");
    ...
    expect(zhLocale).not.toContain("reasoningSummary: '正在梳理'");
    ...
    expect(useTargetChat).toContain('const handleEditLastUserMessage = async (messageId: string, nextContent: string) => {');
