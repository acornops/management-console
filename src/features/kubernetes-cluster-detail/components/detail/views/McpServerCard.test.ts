...
describe('McpServerCard desktop density', () => {
  it('keeps management actions easy to find while quieting repeated secondary context', () => {
    ...
    expect(mcpServerCard).toContain('line-clamp-2 break-words');
    ...
    expect(mcpServerCard).toContain('disabled={!canEditServers}');
    ...
    expect(mcpServerCard).toContain("isManagedServer ? t('mcpServers.managed') : formatDiscoveryTimestamp(server.lastDiscoveryAt)");
