...
interface KnowledgeBankResetDialogProps {
  workspaceId: string;
  targetId: string;
...
export const KnowledgeBankResetDialog: React.FC<KnowledgeBankResetDialogProps> = ({
  workspaceId,
  targetId,
  tool,
  canEdit,
  ...
  return (
    <Dialog
      titleId="knowledge-bank-reset-dialog-title"
    ...
    >
      <div className="flex items-start justify-between gap-4 border-b border-ui-border bg-ui-bg px-6 py-4">
        <div className="min-w-0">
          <h3 id="knowledge-bank-reset-dialog-title" className="type-panel-title">{t('tools.knowledgeBank.resetTitle')}</h3>
        ...
        <button
          ...
          aria-label={t('tools.knowledgeBank.closeReset')}
        >
          <X className="h-4 w-4" />
        </button>
      </div>
      <div className="px-6 py-5">
        <div className="flex gap-3 rounded-lg border border-status-danger/25 bg-status-danger-soft px-4 py-3 text-status-danger-text">
          ...
          <div>
            <p className="text-sm font-semibold">{t('tools.knowledgeBank.resetWarningTitle')}</p>
            <p className="type-caption mt-1">{t('tools.knowledgeBank.resetConfirm')}</p>
          </div>
        </div>
        {error && (
      ...
      <div className="flex items-center justify-end gap-3 border-t border-ui-border bg-ui-bg px-6 py-4">
        {!canEdit ? (
          ...
          <>
            <Button variant="secondary" size="sm" onClick={onClose} disabled={saving}>{t('tools.knowledgeBank.cancel')}</Button>
            <Button variant="danger" size="sm" onClick={() => void resetBank()} disabled={saving}>
              {saving ? t('common.saving') : t('tools.knowledgeBank.reset')}
            </Button>
          </>
        )}
      </div>
