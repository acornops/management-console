...
import { getToolLabel, isManagedMcpServer } from '@/features/kubernetes-cluster-detail/components/detail/views/mcpServersCatalog';
...
export const McpServerToolsDialog: React.FC<{
  ...
  const getConfiguredEnabled = React.useCallback((tool: ClusterToolCatalogItem) => (
  ...
  const getSectionBlockReason = (tools: ClusterToolCatalogItem[]) => {
  ...
  const renderToolRow = (tool: ClusterToolCatalogItem) => {
  ...
  const renderToolSection = (title: string, subtitle: string, tools: ClusterToolCatalogItem[]) => {
