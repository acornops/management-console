...
function mapTool(tool: McpTool): TargetToolCatalogItem {
  return {
    name: tool.toolId,
...
export function flattenCatalogTools(catalog: TargetToolCatalog): TargetMcpToolSummary[] {
  const tools: TargetMcpToolSummary[] = [];
...
export function getToolLabel(tool: TargetToolCatalogItem): string {
  return tool.description || `Tool ${tool.name}`;
}

export function isManagedMcpServer(server: Pick<TargetToolCatalogServer, 'type' | 'isSystem' | 'url'>): boolean {
