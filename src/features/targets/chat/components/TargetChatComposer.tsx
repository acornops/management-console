  ...
  return (
    ...
    >
      <div ref={composerRootRef} className={`${isPanel ? 'max-w-2xl' : 'max-w-3xl'} relative mx-auto`}>
              ...
              {composerAttachments.length > 0 && (
                <div className="flex max-h-32 gap-2 overflow-x-auto custom-scrollbar" role="list" aria-label={t('chat.attachments')}>
                  {composerAttachments.map((attachment) => {
                    ...
                    const statusLabel =
                      attachment.status === 'ready' ? t('chat.attachmentIncluded') : attachment.status === 'error' ? t('chat.attachmentFailed') : t('chat.attachmentPreviewOnly');
                    return (
                      <div
                        key={attachment.id}
                        role="listitem"
              ...
              <div ref={modelMenuRef} className="relative">
                ...
                <AnimatePresence>
                  {isModelMenuOpen && (
                    ...
                    >
                      <p className="px-3 pb-1.5 pt-2 type-label">{t('chat.effortSelector')}</p>
        ...
        <p className="mt-2 text-center type-caption">{canPost ? composerSubmitUnavailableReason || t(resolvedFooterKey) : blockedFooterMessage}</p>
      </div>
    </form>
  );
};
