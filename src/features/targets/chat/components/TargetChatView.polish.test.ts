...
describe('target chat polish contracts', () => {
  ...
  it('keeps polish shared across transcript, composer, and trace surfaces', () => {
    ...
    expect(appTargetChatRuntime).toContain('useConversationAssistantStatuses({');
    expect(appTargetChatRuntime).toContain('isChatVisible: isChatActive');
    expect(appTargetChatRuntime).toContain('sessionAssistantStatuses');
    ...
    expect(chatView).toContain('data-chat-new-chat="true"');
    expect(chatView).toContain("'stable-scrollbar-gutter overflow-y-auto bg-ui-bg px-[var(--ao-route-padding-x)] py-[var(--ao-route-padding-y)] custom-scrollbar'");
    ...
    expect(enLocale).toContain("footerApprovalNotRequired: 'Read and write operations can run automatically under this target policy.'");
    expect(zhLocale).toContain("footerApprovalRequired: '只读操作可自动执行。写操作必须经过明确批准后才会执行。'");
    expect(zhLocale).toContain("footerApprovalNotRequired: '根据此目标策略，只读和写操作都可以自动执行。'");
