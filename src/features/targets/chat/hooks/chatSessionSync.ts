...
export function useControlPlaneChatSessionSync(args: {
  ...
  useEffect(() => {
    latestSessionsRef.current = target.chatSessions;
  }, [target.chatSessions]);
  ...
  useEffect(() => {
    ...
    const loadSessions = async () => {
      ...
      try {
        ...
        const page = await listSessions(target.workspaceId, target.id, { limit: 50 });
  ...
  }, [target.id, target.workspaceId, sessionListKey]);
