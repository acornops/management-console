...
import type { TargetChatController, UseTargetChatArgs } from '@/features/targets/chat/hooks/targetChatControllerTypes';
import { useTargetChatActivityStream } from '@/features/targets/chat/hooks/targetChatActivityStream';
...
export function useTargetChat({
  ...
  const handleLoadEarlierMessages = async () => {
    ...
    try {
      ...
      onUpdateSessions(upsertSession(target.chatSessions, nextSession));
  ...
  const handleDismissRecentActivityWarning = (sessionId?: string) => {
    ...
    onUpdateSessions(target.chatSessions.map((session) =>
  ...
  const handleOpenRecentActivitySession = (sessionId: string) => {
    if (!target.chatSessions.some((session) => session.id === sessionId)) {
  ...
  const handleDeleteSession = async (sessionId: string) => {
    const sessionToDelete = target.chatSessions.find((session) => session.id === sessionId);
    if (!sessionToDelete) {
      return;
    }
  ...
  const handleSendInNewSession = async (overrideInput: string, runtimeSelection?: ChatRuntimeSelection) => {
    ...
    try {
      ...
      if (draftSession.recentActivityWarning) {
        onUpdateSessions(sortSessionsByTimestamp([draftSession, ...target.chatSessions]));
