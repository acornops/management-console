...
expect(!packageJson.scripts.validate.includes('npm run smoke:fixtures'), 'Canonical local validate script should not run browser fixtures');
...
expect(Boolean(packageJson.scripts?.['validate:ci:preflight']), 'package.json should expose a CI preflight validation script');
expectIncludes(packageJson.scripts['validate:ci:preflight'], 'npm run lint', 'CI preflight validation script');
expectIncludes(packageJson.scripts['validate:ci:preflight'], 'npm run test:coverage', 'CI preflight validation script');
expect(!packageJson.scripts['validate:ci:preflight'].includes('npm run design:snapshots'), 'CI preflight should not run browser snapshots');
expect(!packageJson.scripts['validate:ci:preflight'].includes('npm run design:routes'), 'CI preflight should not run browser route checks');
expect(!packageJson.scripts['validate:ci:preflight'].includes('npm run smoke:fixtures'), 'CI preflight should not run browser fixtures');
expect(!packageJson.scripts['validate:ci:preflight'].includes('npm run smoke:mcp-parity'), 'CI preflight should not run MCP parity browser checks');
expect(Boolean(packageJson.scripts?.['validate:ci:browser']), 'package.json should expose a CI browser validation script');
expectIncludes(packageJson.scripts['validate:ci:browser'], 'npm run design:snapshots', 'CI browser validation script');
expectIncludes(packageJson.scripts['validate:ci:browser'], 'npm run design:routes', 'CI browser validation script');
expectIncludes(packageJson.scripts['validate:ci:browser'], 'npm run smoke:fixtures', 'CI browser validation script');
expectIncludes(packageJson.scripts['validate:ci:browser'], 'npm run smoke:mcp-parity', 'CI browser validation script');
expectIncludes(packageJson.scripts['validate:ci'], 'npm run validate:ci:preflight', 'Full CI validation script');
expectIncludes(packageJson.scripts['validate:ci'], 'npm run validate:ci:browser', 'Full CI validation script');
expect(!packageJson.scripts['smoke:fixtures'].includes('--repeat-each'), 'Fixture smoke tests should run once per invocation');
expect(!packageJson.scripts['smoke:mcp-parity'].includes('--repeat-each'), 'MCP parity smoke tests should run once per invocation');
