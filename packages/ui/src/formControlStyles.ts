...
export function formTextareaClassName(className?: string): string {
  return twMerge(clsx(
    ...
    'min-h-32 resize-y bg-ui-bg/70 px-4 py-3 shadow-[inset_0_1px_0_rgb(var(--ao-surface-rgb)/0.95),inset_0_0_0_1px_rgb(var(--ao-border-rgb)/0.35),0_1px_2px_rgb(var(--ao-text-rgb)/0.04)] placeholder:leading-6 focus:bg-ui-surface',
    className
  ));
}
