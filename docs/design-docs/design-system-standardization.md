...
Kubernetes clusters, virtual machines, and agents share one resource-card catalog layout contract. Their catalog wrapper uses `resource-card-catalog` and their card or loading layout uses `resource-card-grid`. The grid reserves as many columns as fit at the `27rem` preferred card minimum and divides the available row width evenly across those columns. Cards fill their tracks without a fixed maximum, while `auto-fill` keeps empty track capacity reserved so sparse catalogs do not expand into space intended for another card. When only one minimum-width column fits, it spans the full catalog width. This intrinsic sizing responds to browser zoom, the desktop sidebar, and docked assistants without feature-specific breakpoints. A docked assistant derives its preferred width from the rendered card so opening it preserves card width; other feature-specific size overrides require a documented design-system exception.
