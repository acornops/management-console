...
| Route and navigation stability | 5/5 | Shared route utilities, route-backed Workflows/Schedules/Activity and Webhooks direction tabs, workflow-level Inbound Webhooks, legacy redirects, URL-backed filters, and exact-run deep links | Continue requiring route tests for new URL state |
| Workflow activity visibility | 4/5 | Workflows-integrated Activity ledger, open-run view count, provenance, webhook execution pointers, and deterministic fixtures | Uses bounded visible-window polling rather than a workspace event stream |
